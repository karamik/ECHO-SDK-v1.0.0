// echo-sdk/src/main/java/com/echo/sdk/core/EchoEconomyManager.kt
...
/**
 * Управляет токенами "Эфир", репутацией узлов и обработкой Relay Receipts.
 * @param tokenDao DAO для балансов.
 * @param receiptDao DAO для обработанных квитанций (защита от replay).
 * @param crypto криптоядро для верификации подписей.
 */
class EchoEconomyManager(
    ...
    suspend fun init() {
        ...
        // Репутация по умолчанию = 50 (нейтральная)
    }

    /**
     * Начисляет награду за ретрансляцию. Идемпотентно.
     * @param receipt квитанция, подписанная получателем.
     * @return true если награда начислена впервые, false если дубликат или подпись неверна.
     */
    suspend fun rewardRelay(receipt: RelayReceipt): Boolean {
        ...
        if (!isValid) {
            penalize(relayHex, 10) // штраф за подделку
            return false
        }
        ...
        return mutex.withLock {
            ...
            try {
                ...
                // Конфликт уникальности (одновременная вставка)
                processedCache.add(cacheKey)
                false
            }
        }
    ...
    /**
     * Уменьшает баланс и репутацию узла (штраф за спам/подделку).
     */
...
/**
 * Квитанция о ретрансляции.
 * @param messageId хэш сообщения.
 * @param relayPublicKey публичный ключ ретранслятора (Ed25519).
 * @param recipientPublicKey публичный ключ получателя (Ed25519).
 * @param timestamp время создания.
 * @param signature подпись получателя (Ed25519) над сериализованными полями (без сигнатуры).
 */
